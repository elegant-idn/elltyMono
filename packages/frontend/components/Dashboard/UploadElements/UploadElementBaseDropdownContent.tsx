...
interface UploadElementBaseDropdownContentProps {
  upload?: any;
  onUploadUpdate?: (id: string, newProperties: Record<string, any>) => unknown;
  children?: React.ReactNode;
}

export const UploadElementBaseDropdownContent: React.FC<
  UploadElementBaseDropdownContentProps
> = ({ upload, onUploadUpdate, children }) => {
  const { t } = useTranslation("common");
  const [cookie] = useCookies();

  const [editingTitle, setEditingTitle] = useState(false);
  const [uploadName, setUploadName] = useState(upload?.title ?? "");
  const timeText = useElapsingTime(new Date(upload?.createdAt));

  const handleSaveTitle = async () => {
    if (!cookie.user_token || !upload._id) return;
    onUploadUpdate?.(upload._id, { title: uploadName });
    ...
    await Api.patch(`/uploads/${upload._id}/`, axiosBody, {
      headers: {
    ...
    <div className={s.root}>
      <div className={clsx(s.header, s.xPadding)}>
        {editingTitle ? (
          ...
          >
            <div className={s.title}>{uploadName}</div>
            <svg
              width="17"
              height="17"
              viewBox="0 0 17 17"
