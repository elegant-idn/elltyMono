...
import DesignElementBaseDropdownContent from "../Dashboard/DesignElements/BaseDropdownContent";
...
import s from "./DeisngElementTrashDropdown.module.scss";

interface DesignElementTrashDropdownProps
  extends Pick<
    ComponentProps<typeof DesignElementBaseDropdownContent>,
    "onTemplateUpdate" | "template"
  > {
  onDelete: (template: any) => unknown;
  onRestore: (template: any) => unknown;

  onDeletingChange?: (deleting: boolean) => unknown;
}

export const DesignElementTrashDropdownContent: React.FC<
  DesignElementTrashDropdownProps
> = ({ template, onTemplateUpdate, onDelete, onRestore, onDeletingChange }) => {
  ...
  if (isDeleting) {
    return (
      <div className={s.deleteRoot}>
        <p className={s.deleteTitle}>
          {tDashboard("deleteTemplateModal.title")}
        </p>
        <p className={s.deleteBody}>
          {tDashboard("deleteTemplateModal.description")}
        </p>
        <div className={s.deleteActions}>
          ...
          >
            {tDashboard("deleteTemplateModal.cancel")}
          </BtnOutline>
          <BtnOutline
            variant="root"
          ...
          >
            {tDashboard("deleteTemplateModal.submit")}
          </BtnOutline>
        </div>
      </div>
    );
    ...
    >
      <DropdownActions>
        {actions.map((action, i) => {
          return (
            <button onClick={action.onClick as any} key={i}>
              {action.icon}
