...
const TemplatesPage: NextPage<TemplatesPageProps> = ({
  cookieUser,
  userToken,
}) => {
  ...
  const inputImageRef = React.useRef<any>();
  const [imageValue, setImageValue] = React.useState({});
  const [imageValueName, setImageValueName] = React.useState("");
  const [imagePreview, setImagePreview] = React.useState("");

  const [typeCheckbox, setTypeCheckbox] = React.useState<string>("free"); // free/pro
  const [languageCheckbox, setLanguageCheckbox] = React.useState<any>([]);

  // console.log(languageCheckbox.includes('en'));
  ...
  const [languageCheckboxIsDirty, setLanguageCheckboxIsDirty] =
    React.useState(false);
  ...
  React.useEffect(() => {
    Api.get(`/templates?amount=${elementsOnPage}&page=${currentPage}`)
      .then((result) => {
  ...
  React.useEffect(() => {
    Api.get(`/templates?amount=${elementsOnPage}&page=0`)
      .then((result) => {
        // console.log(result.data);
        setTemplates(result.data.templates);
        setTemplatesTotal(result.data.totalItems);
    ...
    <PageLayout userToken={userToken}>
      ...
      >
        <div className={s.root}>
          <div className={s.blockTitle}>{i18nTemplatesPage.title}</div>

          {!addingElem && !editingElem ? (
            <>
              {elemAdded && (
                <Alert variant="success">{i18nTemplatesPage.success}</Alert>
              )}
              ...
              <div className={s.addBtn}>
                ...
                >
                  {i18nTemplatesPage.addTemplate}
                </BtnSecondary>
              </div>

              <div className={s.blockSubtitle}>
                {i18nTemplatesPage.allTemplates} ({templatesTotal})
              </div>
              ...
              <ConfirmationModal
                ...
                onConfirm={() => {
                  ...
                  Api.delete("/templates/delete", body)
                    .then((result) => {
                      setSelectedElements([]);
            ...
            <>
              <div className={s.blockSubtitle}>
                {i18nTemplatesPage.addNewTemplate}
              </div>
              ...
              <Formik
                ...
                onSubmit={(values, actions) => {
                  ...
                  const tags = tagSelectValue
                    ...
                    .join(",");
                  // const tags = tagSelectValue;
                  const languages = languageCheckbox.join(",");
                  ...
                  // @ts-ignore
                  formData.append("json", jsonValue);
                  // @ts-ignore
                  ...
                  languageCheckboxIsDirty &&
                    editFormData.append("languages", languages);
                  colorsFieldIsDirty && editFormData.append("colors", colors);
                  typeCheckboxIsDirty &&
                    editFormData.append("status", typeCheckbox);
                  // @ts-ignore
                  jsonFieldIsDirty && editFormData.append("json", jsonValue);
                  // console.log(jsonSizes);
                  jsonFieldIsDirty &&
                    // @ts-ignore
                    editFormData.append("width", String(jsonSizes.width));
                  jsonFieldIsDirty &&
                    // @ts-ignore
                    editFormData.append("height", String(jsonSizes.height));
                  // @ts-ignore
                  imageFieldIsDirty && editFormData.append("image", imageValue);
                  // @ts-ignore
                  ...
                  editingElem
                    ? Api.put(
                        `/templates/edit/${selectedElements[0]}`,
                        editFormData,
                        config
                      )
                        .then((result) => {
                    ...
                    : Api.post("/templates/create", formData, config)
                        .then((result) => {
                          // console.log(result);
                          if (result.status === 201) {
                            ...
                            Api.get(
                              `/templates?amount=${elementsOnPage}&page=0`
                            )
                              .then((result) => {
                                // console.log(result);
                                setTemplates(result.data.templates);
                ...
                {(props) => (
                  <div className={s.wrapper}>
                    <form onSubmit={props.handleSubmit} className={s.form}>
                      ...
                      <div className={s.inputGroup}>
                        <label>{i18nTemplatesPage.languages}</label>
                        <div className={s.typeCheckboxWrapper}>
                          {data.langList.map((item) => {
                            return (
                              <>
                                <span>{item.abbr.toLocaleUpperCase()}</span>
                                <InputCheckbox
                                  onChange={() => {
                                    if (languageCheckbox.includes(item.abbr)) {
                                      setLanguageCheckbox(
                                        languageCheckbox.filter(
                                          (s: any) => s !== item.abbr
                                        )
                                      );
                                    } else {
                                    ...
                                    setLanguageCheckboxIsDirty(true);
                                  }}
                                  value={"type"}
                                  checked={languageCheckbox.includes(item.abbr)}
                                  // checked={false}
                                  variant="bw"
                                />
                              </>
                        ...
                        <div className={s.uploadGroup}>
                          ...
                          <input
                            ...
                            onChange={(event) => {
                              ...
                              // @ts-ignore
                              new Response(event.currentTarget.files[0])
                                .json()
                                .then(
                                  (json) => {
                                    // setSaveElemErrorText('')
                      ...
                      <div className={s.uploadFileGroup}>
                        <div className={s.labelText}>
                          {i18nTemplatesPage.previewTitle}
                        </div>

                        <div className={s.uploadGroup}>
                          {imageValueName && (
                            <div className={s.fileName}>{imageValueName}</div>
                          )}
